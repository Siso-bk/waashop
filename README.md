...
An ecommerce + virtual wallet experience for Telegram Mini Apps. Users spend in-app MINI on mystery boxes, receive guaranteed MINIS, and every purchase is processed server-side with crypto-secure randomness and Mongo-backed ledgers.
...
| `POST` | `/api/boxes/buy` | Atomic purchase; deducts MINIS, runs secure random reward, writes ledgers/purchase. Body `{ boxId, purchaseId }`. |
...
- **Transparent rewards**: Each mystery box exposes its tier probabilities; the API enforces crypto-secure randomness and guarantees a minimum MINI payout.
- **Top-tier cooldown**: Users can only hit the top tier every 7 days; otherwise they’re downgraded to the next best tier.
- **Wallet ledger**: Every purchase writes a MINI debit + MINI credit entry so `/wallet` stays auditable.
